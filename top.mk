...
ifeq ($(OS), Windows_NT)
	...
	ifeq ($(PLATFORM), linux)
		...
		ifeq ($(TYPE), dll)
			...
			LINK_REL = g++ -Wl,--relax -Wl,--gc-sections -Wl,-Map=$(OBJDIR_REL)/$(TARGET).map,--cref -o $@ $(OBJS_REL) $(TESTOBJS_REL) $(foreach DEP,$(DEPS),-L$(ROOT)/libs/lib$(DEP)/$(OUTDIR_REL) -l$(DEP)); strip $@; for i in $(DLLS_REL); do cp -rp $$i $(OUTDIR_REL); done
			LINK_DBG = g++ -Wl,--relax -Wl,--gc-sections -Wl,-Map=$(OBJDIR_DBG)/$(TARGET).map,--cref -o $@ $(OBJS_DBG) $(TESTOBJS_DBG) $(foreach DEP,$(DEPS),-L$(ROOT)/libs/lib$(DEP)/$(OUTDIR_DBG) -l$(DEP)); for i in $(DLLS_DBG); do cp -rp $$i $(OUTDIR_DBG); done
		endif
	else
		TARGET = $(PLATFORM)
	endif
...
dbg: $(DEPGETS) $(DEPDIRS) $(OBJDIR_DBG) $(OUTDIR_DBG)/$(TARGET)
	$(RUNTESTS_DBG)

rel: $(DEPGETS) $(DEPDIRS) $(OBJDIR_REL) $(OUTDIR_REL)/$(TARGET)
	$(RUNTESTS_REL)
